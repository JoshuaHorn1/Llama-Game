"""Bug Fixes No.05
- Increases the spawn chance of cacti as the score increases
- Cacti can spawn closer together
"""
...
speed_counter = 0
cacti_delay_reset = 20  # a value that the counter must reach for a new cacti to spawn
...
while running:
    ...
    if not game_over and not resetting_game:
        ...
        if cacti_delay_counter >= cacti_delay_reset:  # check if a cacti has been placed recently
            ...
            if random.randint(0, 600) < 3 or cacti_force_spawn == 150:
                cacti_force_spawn = 0
